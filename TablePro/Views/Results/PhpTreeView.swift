...
internal struct PhpTreeView: View {
    ...
    private var filteredRootNodes: [PhpTreeNode] {
    ...
    private static func filterNodes(_ nodes: [PhpTreeNode], matching query: String) -> [PhpTreeNode] {
        nodes.compactMap { node in
            ...
            if !filteredChildren.isEmpty {
                return PhpTreeNode(
                    id: node.id, key: node.key, keyPath: node.keyPath,
                    nodeType: node.nodeType, displayValue: node.displayValue,
                    visibilityBadge: node.visibilityBadge, children: filteredChildren
                )
            }
            if keyMatches || valueMatches {
                return PhpTreeNode(
                    id: node.id, key: node.key, keyPath: node.keyPath,
                    nodeType: node.nodeType, displayValue: node.displayValue,
                    visibilityBadge: node.visibilityBadge, children: []
                )
            }
            return nil
        }
    ...
    private func collectMatchingContainerIDs(_ nodes: [PhpTreeNode]) -> Set<UUID> {
        var ids: Set<UUID> = []
    ...
    private func collectAllContainerIDs(_ node: PhpTreeNode) -> Set<UUID> {
        var ids: Set<UUID> = []
...
private struct PhpTreeContentView: View {
    let nodes: [PhpTreeNode]
    ...
    @ViewBuilder
    private func nodeContextMenu(for node: PhpTreeNode) -> some View {
        Button(String(localized: "Copy Value")) {
            ClipboardService.shared.writeText(node.displayValue)
        }
...
private struct PhpTreeRowView: View {
    ...
    private var accessibilityLabel: String {
        let prefix = node.key ?? ""
        let head = (node.displayValue as NSString).length > 120
            ? (node.displayValue as NSString).substring(to: 120)
            : node.displayValue
        return "\(prefix), \(node.nodeType.badgeLabel), \(head)"
    }
}
